...
from utils.pushToPhone import pushToIos
...
def login(user, passwd, urlLogin='https://uis.nwpu.edu.cn/cas/login'):
    ...
    loginData = {
        ...
        'password': passwd,
        'currentMenu': 1,
        'execution': re.findall("""name="execution" value="(.*?)"/>""", session.get(urlLogin).text)[0],  
        '_eventId': 'submit',
        'mfaState': mfastate,
        'geolocation': '',
        'submit': '稍等片刻……'
    ...
    response = session.post(url=urlLogin, data=loginData, headers=header)
    ...
    if('欢迎使用' in res):
        ...
        sys.exit()
    elif('var hasErrors = true;' in res):
        print('Name or Password Wrong!')
        pushToIos('Name or Password Wrong!')
        sys.exit()
    else:
        print('Login failed, Please check')
        pushToIos('Login failed, Please check')
        sys.exit()
